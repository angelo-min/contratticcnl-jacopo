...
import { getGuidesWithTabelle, hasTabellaRetributivaStrutturata } from '@/data/db'
...
export default function TabelleRetributiveIndexPage() {
  const guide = getGuidesWithTabelle()
      ...
      <main className="flex-1">
        ...
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {guide.map((g) => {
              const hasStrutturata = hasTabellaRetributivaStrutturata(g.slug)
              return (
                <Link
                  key={g.slug}
                  ...
                  className={cn(
                    ...
                    hasStrutturata ? 'opacity-100' : 'opacity-40 hover:opacity-100',
                  )}
                >
          ...
          <p className="mt-10 text-sm text-muted-foreground">
            Stiamo progressivamente aggiungendo le tabelle strutturate per tutti i CCNL.
            Per il catalogo completo consulta{' '}
            <Link href="/contratti-ccnl" className="text-primary hover:underline">
